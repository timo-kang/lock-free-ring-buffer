# Lock-Free IPC 마이그레이션 제안서

## 1. 배경: 현재 Mutex 기반 공유 메모리의 문제점

### 1.1 현재 아키텍처

기존 시스템의 로컬 IPC는 `SharedMemoryPublisher<T>` / `SharedMemorySubscriber<T>`를 통해 구현되어 있다.
...
기존 시스템의 `Publisher<T>` / `Subscriber<T>` 인터페이스와 호환되는 어댑터를 구현했다:
...
어댑터가 제공하는 기능:
- `publish()` / `subscribe()` — 기존 시스템과 동일한 사용 패턴
...
가변 크기 필드(std::vector, std::string)를 포함하는 기존 시스템 메시지를 `SharedLatest<T>`에서 사용하기 위해 고정 크기 래퍼를 구현했다:
...
| 항목 | Mutex-IPC (현재) | Lock-free IPC (제안) |
|------|-------------|---------------|
...
|-----------|----------------:|---------:|------|
| **Mutex 기반 공유 메모리** | **719 ns** | **736 ns** | mutex + futex |
| **Lock-free (seqlock)** | **71 ns** | **57 ns** | seqlock |
...
|-----------|----------------:|------|
| **Mutex 기반 공유 메모리** | **1.45 μs** | mutex + serialize + futex_wake + kernel wake |
| **Lock-free (seqlock)** | **1.10 μs** | seqlock + memcpy + futex_wake + kernel wake |

**Spin 경로: SharedLatest는 Mutex-IPC 대비 10.1~12.8배 빠르다.**
**Futex 경로: SharedLatest는 Mutex-IPC 대비 1.3배 빠르다** (커널 futex round-trip ~1μs가 지배적).

Mutex-IPC의 레이턴시가 페이로드 크기(16B vs 80B)에 무관하게 ~719-736ns로 일정한 것은 mutex syscall 오버헤드가 지배적이기 때문이다. Lock-free IPC은 커널 전환이 없으므로 페이로드 크기에 비례하는 합리적인 성능을 보인다. Futex 경로에서는 커널 context switch 비용(~500ns)이 양쪽 모두 동일하므로, 전송 속도 차이가 압축되어 1.3배로 나타난다.
...
- Mutex-IPC futex handoff: 1.45μs → 주기의 **0.07%**
- Lock-free futex handoff: 1.10μs → 주기의 **0.06%**
- Lock-free spin handoff: 71ns → 주기의 **0.004%**
...
로봇 제어 프레임워크 코드베이스에서 확인된 전체 메시지 타입:
...
| 토픽 | 메시지 타입 | 크기 | 빈도 | Publisher → Subscriber | Lock-free 구조체 |
|------|-----------|-----:|------|----------------------|----------------|
...
Phase 1.5 ✅ 완료: Futex Notification + SCHED_FIFO Benchmarks
   └── SharedLatest에 futex 기반 알림 기능 추가
       ...
       ├── benchmark_raisin_shm.cpp (Mutex-IPC vs Lock-free futex 비교)
       ├── stress_test.cpp (crash recovery + contention + 센서 루프 시뮬레이션)
       └── 결과: futex handoff 1.10μs (Lock-free IPC) vs 1.45μs (Mutex-IPC), SCHED_FIFO 하
               crash recovery 510ns vs 20.9μs, 센서 루프 e2e 2.5-7.1x 개선
               (Mutex-IPC 2kHz 422회/1kHz 78회 deadline 초과, Lock-free 전 구간 0회)
...
Phase 6: 정리
   └── 마이그레이션 완료된 토픽의 기존 Mutex 기반 SharedMemory 코드 제거
```

### 5.4 예상 효과 요약

| 지표 | 현재 (Mutex-IPC) | 마이그레이션 후 (Lock-free IPC) |
|------|-------------------|------------------------|
