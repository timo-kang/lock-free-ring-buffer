...
A C++20 lock-free ring buffer for shared-memory messaging. The current implementation is **MPSC** (multi‑producer, single‑consumer) with variable‑size payloads and a memory‑mapped file backend. It is designed to minimize false sharing with cache‑line padded control fields.
...
## Status / Roadmap
- MPMC support: **planned** (mandatory for broader use).
